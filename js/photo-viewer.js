import { armLongPressReleaseGuard, consumeLongPressSyntheticClick } from "./long-press-guard.js?v=20260922-searchcards33";
...
import { getBestPhotoUrl, getPhotoPreviewUrl, escapeHtml } from "./helpers.js?v=20260922-searchcards33";
import { getOwnerId } from "./group-context.js?v=20260922-searchcards33";
import {
    showPhotoViewerScreen,
    pushPhotoHistory
} from "./navigation.js?v=20260922-searchcards33";
import { photoCommentOwnerId } from "./photo-comment-api.js?v=20260922-searchcards33";
import { openVkProfile, openVkTarget, openVkPhoto } from "./vk-links.js?v=20260922-searchcards33";
import { invalidatePhotoActivityCaches } from "./cache.js?v=20260922-searchcards33";
...
export function initPhotoViewer() {
    ...
    window.addEventListener("photo-data-updated", event => {
        const updated = event?.detail?.photo;
        if (!updated || !activePhoto) return;
        if (Number(updated.id) !== Number(activePhoto.id)) return;
        activePhoto = { ...activePhoto, ...updated };
        state.currentPhoto = activePhoto;
